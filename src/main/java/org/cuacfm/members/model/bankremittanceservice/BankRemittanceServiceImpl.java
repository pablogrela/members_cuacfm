...
public class BankRemittanceServiceImpl implements BankRemittanceService {
   ...
   public void createBankRemittance(Date dateCharge, Date monthCharge) {
      ...
      Map<Account, List<PayMember>> accountPayMembers = payMemberService
            .getPayMemberNoPayListByDirectDebit(monthCharge);

      // Direct debit of fee program
      Map<Account, List<PayProgram>> accountPayPrograms = payProgramService
            .getPayProgramNoPayListByDirectDebit(monthCharge);

      for (Account account : accountService.getUsersDirectDebit()) {
         if (account.activeBankAccount() != null
               && (accountPayMembers.get(account) != null || accountPayPrograms.get(account) != null)) {
            ...
            if (payPrograms != null) {
               for (PayProgram payProgram : payPrograms) {
                  ...
                  concept = concept + ", " + payProgram.getFeeProgram().getName() + " ["
                        + payProgram.getProgram().getName() + "]";
               }
            }
            ...
            DirectDebit directDebit = new DirectDebit(account, bankRemittance, payMembers,
                  payPrograms, price, concept, account.activeBankAccount().getMandate(),
                  directDebitRepository.isRcurOrFRST(account.getId()));
    ...
    */
   public void updateStateBankRemittance(Long bankRemittanceId, states state, methods method)
         throws ExistTransactionIdException {
      ...
      for (DirectDebit directDebit : directDebitRepository
            .getDirectDebitListByBankRemittanceId(bankRemittance.getId())) {
         if (!directDebit.getState().equals(states.RETURN_BILL)) {
            updateStateDirectDebit(directDebit.getId(), state, method,
                  bankRemittance.getDateCharge());
         }
      }
   }
    ...
    *            the exist transaction id exception
    */
   public void updateStateDirectDebit(Long directDebitId, states state, methods method, Date datePay)
         throws ExistTransactionIdException {
   ...
   public ResponseEntity<byte[]> createTxtBankRemittance(MessageSource messageSource,
      ...
      try {
         new CreatePayRoll(path, messageSource, bankRemittance,
               directDebitRepository.getDirectDebitListByBankRemittanceId(bankRemittanceId));
      } catch (IOException e) {
         e.getMessage();
      }
