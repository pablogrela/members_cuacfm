...
public class FeeProgramEditTest extends WebSecurityConfigurationAware {
   ...
   public void postDisplaysInscriptionsTest() throws Exception {
      ...
      mockMvc.perform(
            post("/feeProgramList/feeProgramEdit/" + feeProgram.getId()).locale(Locale.ENGLISH)
                  .session(defaultSession)).andExpect(
            view().name("redirect:/feeProgramList/feeProgramEdit"));
   }
   ...
   public void redirectFeeProgramListBecauseFeeProgramIsNullTest() throws Exception {
      mockMvc.perform(
            post("/feeProgramList/feeProgramEdit/" + Long.valueOf(0)).locale(Locale.ENGLISH)
                  .session(defaultSession)).andExpect(
            view().name("redirect:/feeProgramList/feeProgramEdit"));

      mockMvc.perform(
            get("/feeProgramList/feeProgramEdit").locale(Locale.ENGLISH).session(defaultSession))
   ...
   public void postFeeProgramEditSuccesfulTest() throws Exception {
      ...
      mockMvc.perform(
            post("/feeProgramList/feeProgramEdit").locale(Locale.ENGLISH).session(defaultSession)
                  .param("name", "Pay april 2015").param("price", "24").param("date", "2015-04")
   ...
   public void maxCharactersFeeProgramEditTest() throws Exception {
      ...
      mockMvc
            .perform(
                  post("/feeProgramList/feeProgramEdit").locale(Locale.ENGLISH)
                        .session(defaultSession)
                        .param("name", "111111111111111111111111111111111111111111111111111111111111111111111111111111111111111")
