...
public class PayMemberListControllerTest extends WebSecurityConfigurationAware {
	...
	public void displayInscriptionsViewPageWithoutSiginInTest() throws Exception {
		mockMvc.perform(get("/feeMemberList/payMemberList")).andExpect(redirectedUrl("http://localhost/signin"));
	}
	...
	public void displaysInscriptionsTest() throws Exception {
		...
		mockMvc.perform(post("/feeMemberList/payMemberList/" + feeMember.getId()).locale(Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("redirect:/feeMemberList/payMemberList"));

		mockMvc.perform(get("/feeMemberList/payMemberList").locale(Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("paymember/paymemberlist")).andExpect(content().string(containsString("<title>Payments fee member</title>")));
	}
	...
	public void redirectTrainingListBecauseTrainingIsNullTest() throws Exception {
		...
		mockMvc.perform(post("/feeMemberList/payMemberList/" + Long.valueOf(0)).locale(Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("redirect:/feeMemberList/payMemberList"));
	...
	public void payPayMemberTest() throws Exception {
		...
		PayMember payMember = payMemberService.findByPayMemberIds(user.getId(), feeMember.getId()).get(0);
		// Assert no pay
		...
		mockMvc.perform(post("/feeMemberList/payMemberList/pay/" + payMember.getId()).locale(Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("redirect:/feeMemberList/payMemberList"));

		// Assert Pay
		assertTrue(payMemberService.findByPayMemberIds(user.getId(), feeMember.getId()).get(0).getState().equals(states.PAY));
	}
	...
	public void blankMessageLoginTest() throws Exception {
		...
		mockMvc.perform(post("/feeMemberList/payMemberList").locale(Locale.ENGLISH).session(defaultSession).param("login", " "))
				.andExpect(content().string(containsString("The value may not be empty!"))).andExpect(view().name("paymember/paymemberlist"));
	}
	...
	public void noExistLoginTest() throws Exception {
		mockMvc.perform(post("/feeMemberList/payMemberList").locale(Locale.ENGLISH).session(defaultSession).param("login", "No exist login"))
				.andExpect(content().string(containsString("No exist login"))).andExpect(view().name("paymember/paymemberlist"));
	}
	...
	public void loginAlreadyExistTest() throws Exception {
		...
		mockMvc.perform(post("/feeMemberList/payMemberList").locale(Locale.ENGLISH).session(defaultSession).param("login",
				user.getId() + ": " + user.getLogin())).andExpect(content().string(containsString("It has Already created to pay him the")))
	...
	public void addUserToFeeMemberTest() throws Exception {
		...
		accountService.update(user2, false, false);

		mockMvc.perform(post("/feeMemberList/payMemberList").locale(Locale.ENGLISH).session(defaultSession).param("login",
				user2.getId() + ": " + user.getLogin())).andExpect(view().name("redirect:/feeMemberList/payMemberList"));
	}
	...
	public void createPdfALL() throws Exception {
		Account user2 = new Account("user2", "25555555C", "London", "user2", "user2@udc.es", "666666666", "666666666", "demo", roles.ROLE_USER);
		accountService.save(user2);
		Account user3 = new Account("user3", "35555555C", "London", "user3", "user3@udc.es", "666666666", "666666666", "demo", roles.ROLE_USER);
		user3.setInstallments(2);
		accountService.save(user3);
		Account user4 = new Account("user4", "45555555C", "London", "user4", "user4@udc.es", "666666666", "666666666", "demo", roles.ROLE_USER);
		...
		mockMvc.perform(post("/feeMemberList/payMemberList/" + feeMember2.getId()).locale(Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("redirect:/feeMemberList/payMemberList"));

		mockMvc.perform(post("/feeMemberList/payMemberList/createPdf/" + feeMember.getId()).locale(Locale.ENGLISH).session(defaultSession)
				.param("createPdf", "ALL"));
	}
