...
public class MethodPaymentCreateControllerTest extends WebSecurityConfigurationAware {
	...
	public void displayaccountCreateCreatePageWithoutSiginInTest() throws Exception {
		mockMvc.perform(get("/configuration/methodPaymentCreate")).andExpect(redirectedUrl("http://localhost/signin"));
	}
	...
	public void postMethodPaymentCreateTest() throws Exception {
		mockMvc.perform(post("/configuration/methodPaymentCreate").locale(Locale.ENGLISH).session(defaultSession).param("name", "Paypal")
	...
	/**
	 * "Already exist type of formation with name "+ methodPayment.getName() + ", please chose other" Send displaysmethodPaymentList.
	 * 
	 * @throws Exception the exception
	 */
	@Test
	public void maxCharactersTest() throws Exception {
		mockMvc.perform(post("/configuration/methodPaymentCreate").locale(Locale.ENGLISH).session(defaultSession)
				...
				.andExpect(content().string(containsString("Maximum 50 characters"))).andExpect(view().name("configuration/methodpaymentcreate"));
