	...
	public void notOnAndBlanckMessageTest() throws Exception {
		mockMvc.perform(
				post("/profile").locale(Locale.ENGLISH).session(defaultSession)
						...
						.param("onDni", "false").param("dni", "dni")
						.param("onAddress", "false").param("address", "address")
						.param("onPhone", "false").param("phone", "12356789")
						.param("onMobile", "false").param("mobile", "12356789")
						.param("onStudent", "false").param("student", "true")
						.param("onDateBirth", "false").param("dateBirth", "1990-05-02")).andExpect(
				view().name("profile/profile"));
	}
	...
	public void notOnAndMessageTest() throws Exception {
		...
		mockMvc.perform(
				post("/profile").locale(Locale.ENGLISH).session(defaultSession)
						...
						.param("rePassword", "1233")).andExpect(
				view().name("redirect:/profile"));
	}
