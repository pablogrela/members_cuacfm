...
public class AccountTypeEditControllerTest extends WebSecurityConfigurationAware {
   ...
   public void redirectAccountListBecauseAccountTypeIsNullTest() throws Exception {
      mockMvc.perform(
            post("/configuration/accountTypeEdit/" + Long.valueOf(0)).locale(Locale.ENGLISH)
                  .session(defaultSession)).andExpect(
            view().name("redirect:/configuration/accountTypeEdit"));

      mockMvc.perform(
            get("/configuration/accountTypeEdit").locale(Locale.ENGLISH).session(defaultSession))
   ...
   public void displaysAccountTypeEditTest() throws Exception {
      ...
      mockMvc.perform(
            post("/configuration/accountTypeEdit/" + accountType.getId()).locale(Locale.ENGLISH)
                  .session(defaultSession)).andExpect(
            view().name("redirect:/configuration/accountTypeEdit"));
   ...
   public void postAccountTypeEditTest() throws Exception {
      ...
      mockMvc.perform(
            post("/configuration/accountTypeEdit").locale(Locale.ENGLISH).session(defaultSession)
   ...
   public void notBlankMessageInAccountTypeEditTest() throws Exception {
      ...
      mockMvc
            .perform(
                  post("/configuration/accountTypeEdit").locale(Locale.ENGLISH).session(
                        defaultSession))
   ...
   /**
    * "Already exist type of account with name "+ accountType.getName() +
