...
public class AccountControllerTest extends WebSecurityConfigurationAware {
	...
	public void postDisplaysAccountFormTest() throws Exception {
		mockMvc.perform(
				post("/account/" + defaultSession.getId()).locale(
	...
	public void displaysAccount2FormTest() throws Exception {
		...
		mockMvc.perform(
				post("/account/" + user2.getId()).locale(
						Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("redirect:/account"));
		
		mockMvc.perform(
				get("/account").locale(Locale.ENGLISH).session(defaultSession))
	...
	public void displaysaccountFormTest() throws Exception {
		mockMvc.perform(
				post("/account/" + user.getId()).locale(
						Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("redirect:/account"));
	...
	public void redirectAccountListBecauseAccountIsNullTest() throws Exception {    
		mockMvc.perform(
				post("/account/" + +Long.valueOf(0)).locale(
						Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("redirect:/account"));
	...
	public void postdniAlreadyExistsTest() throws Exception {
		...
		mockMvc.perform(
				post("/account").locale(Locale.ENGLISH).session(defaultSession)
						...
						.param("onLogin", "true").param("login", "user")
	...
	public void incorrectEmailFormat() throws Exception {
		...
		mockMvc.perform(
				post("/account").locale(Locale.ENGLISH).session(defaultSession)
						.param("name", "name").param("login", "login")
						.param("email", "email").param("password", "1234")
						.param("rePassword", "1234")
						.param("onInstallments", "true")
						.param("installments", "1"))
				.andExpect(
	...
	public void updateaccountSuccesfullTest() throws Exception {
		...
		mockMvc.perform(
				post("/account").locale(Locale.ENGLISH).session(defaultSession)
						.param("onName", "true").param("name", "name")
						.param("onLogin", "true").param("login", "login")
						.param("onDni", "true").param("dni", "6666666666666666C")
						.param("onAddress", "true").param("address", "France")
						.param("onEmail", "true")
						.param("email", "email2@udc.es")
					    .param("onPhone", "true").param("phone", "12356789")
						.param("onMobile", "true").param("mobile", "12356789")
	...
	public void notOnAndBlanckMessage() throws Exception {
		...
		mockMvc.perform(
				post("/account").locale(Locale.ENGLISH).session(defaultSession)
						...
						.param("onLogin", "false").param("login", " ")
						.param("onEmail", "false").param("email", " ")
						.param("onPassword", "false").param("password", " ")
						.param("onInstallments", "false")
						.param("installments", "1")).andExpect(
				view().name("account/account"));
	}
	...
	public void notOnAndMessage() throws Exception {
		...
		mockMvc.perform(
				post("/account").locale(Locale.ENGLISH).session(defaultSession)
						...
						.param("onPassword", "false").param("password", "1234")
