...
public class AccountControllerTest extends WebSecurityConfigurationAware {
   ...
   public void initializeDefaultSession() throws UniqueException {
	...
	public void postDisplaysAccountFormTest() throws Exception {
		mockMvc.perform(post("/account/" + defaultSession.getId()).locale(Locale.ENGLISH).session(defaultSession))
	...
	public void displaysAccount2FormTest() throws Exception {
		...
		mockMvc.perform(get("/account").locale(Locale.ENGLISH).session(defaultSession)).andExpect(model().attributeExists("profileForm"))
				.andExpect(view().name("account/account")).andExpect(content().string(allOf(containsString("<title>Account user</title>"),
	...
	public void redirectAccountListBecauseAccountIsNullTest() throws Exception {
		mockMvc.perform(post("/account/" + +Long.valueOf(0)).locale(Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("redirect:/account"));

		mockMvc.perform(get("/account").locale(Locale.ENGLISH).session(defaultSession)).andExpect(view().name("redirect:/accountList"));
	}
	...
	public void updateaccountBlankMessageTest() throws Exception {
		...
		mockMvc.perform(
				post("/account").locale(Locale.ENGLISH).session(defaultSession).param("onName", "true").param("name", " ").param("onLogin", "true")
						.param("login", " ").param("mobile", "12356789").param("onEmail", "true").param("email", " ").param("onPassword", "true")
	...
	public void updateaccountSuccesfullTest() throws Exception {
		...
		mockMvc.perform(post("/account").locale(Locale.ENGLISH).session(defaultSession).param("name", "name").param("dni", "dni")
				.param("address", "address").param("cp", "cp").param("mobile", "12356789").param("province", "province").param("codeCountry", "EN")
	...
	public void insuficientCharactersPasswordsTest() throws Exception {
		...
		mockMvc.perform(post("/account").locale(Locale.ENGLISH).session(defaultSession).param("onName", "true").param("name", "name")
				...
				.param("onObservations", "true").param("Observations", "Good partner").param("onRole", "true").param("Role", "ROLE_TRAINER"))
	...
	public void notOnAndBlanckMessage() throws Exception {
		...
		mockMvc.perform(post("/account").locale(Locale.ENGLISH).session(defaultSession).param("onName", "false").param("name", " ")
				.param("onLogin", "false").param("mobile", "12356789").param("login", " ").param("onEmail", "false").param("email", " ")
				.param("onPassword", "false").param("password", " ").param("onInstallments", "false").param("installments", "1"))
				.andExpect(view().name("account/account"));
	}
	...
	public void notOnAndMessage() throws Exception {
		...
		mockMvc.perform(post("/account").locale(Locale.ENGLISH).session(defaultSession).param("onName", "false").param("name", "name")
				.param("onLogin", "false").param("mobile", "12356789").param("login", "login").param("onEmail", "false")
				.param("email", "email@example.es").param("onPassword", "false").param("password", "1234").param("rePassword", "1233")
				.param("onInstallments", "false").param("installments", "1")).andExpect(view().name("account/account"));
	}
	...
	public void createBankAccountExceptionTest() throws Exception {
		...
		mockMvc.perform(post("/bankAccount").locale(Locale.ENGLISH).session(defaultSession).param("bank", "Santander").param("bic", "BSCHESMMXXX")
				.param("iban", "12345678912345678901234")).andExpect(view().name("account/account"));
	}
	...
	public void createBankAccountTest() throws Exception {
		...
		mockMvc.perform(post("/bankAccount").locale(Locale.ENGLISH).session(defaultSession).param("bank", "11111111").param("bic", "").param("iban",
				"ES7620770024003102575766")).andExpect(view().name("account/account"));
	}
