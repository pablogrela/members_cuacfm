...
public class TrainingEditControllerTest extends WebSecurityConfigurationAware {
	...
	public void displayTrainingEditPageWithoutSiginInTest() throws Exception {
		mockMvc.perform(get("/trainingList/trainingEdit")).andExpect(redirectedUrl("http://localhost/signin"));
	}
	...
	public void displaysTrainingEditTest() throws Exception {
		...
		mockMvc.perform(get("/trainingList/trainingEdit").locale(Locale.ENGLISH).session(defaultSession))
				.andExpect(view().name("training/trainingedit")).andExpect(content().string(containsString("<title>Edit session</title>")));
	}
	...
	public void postTrainingEditTest() throws Exception {
		...
		mockMvc.perform(post("/trainingList/trainingEdit").locale(Locale.ENGLISH).session(defaultSession).param("name", "Locution")
	...
	public void dataBlank2TrainingEditTest() throws Exception {
		...
		mockMvc.perform(post("/trainingList/trainingEdit").locale(Locale.ENGLISH).session(defaultSession).param("name", "").param("timeLimit", "")
	...
	public void countPlaceExceptionTest() throws Exception {
		...
		mockMvc.perform(post("/trainingList/trainingEdit").locale(Locale.ENGLISH).session(defaultSession).param("name", "Locution")
				.param("timeLimit", "10:30").param("dateLimit", "2015-12-05").param("timeTraining", "10:30").param("dateTraining", "2015-12-05")
				.param("description", "Very interesting2").param("place", "livingRoom2").param("duration", "90").param("countPlaces", "12")
				.param("maxPlaces", "10").param("close", "false"))
				.andExpect(content().string(containsString("The number of reservations can not exceed 10")))
				.andExpect(view().name("training/trainingedit"));
	}
}