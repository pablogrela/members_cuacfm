...
public class UserPayInscriptionListControllerTest extends WebSecurityConfigurationAware {
   ...
   public void displaysInscriptionsTest() throws Exception {
      ...
      mockMvc.perform(
            post("/payInscriptionList/userPayInscriptionList/" + payInscription.getId()).locale(
                  Locale.ENGLISH).session(defaultSession)).andExpect(
            view().name("redirect:/payInscriptionList/userPayInscriptionList"));

      mockMvc
            .perform(
                  get("/payInscriptionList/userPayInscriptionList").locale(Locale.ENGLISH).session(
                        defaultSession))
   ...
   public void redirectTrainingListBecauseTrainingIsNullTest() throws Exception {
      ...
      mockMvc.perform(
            post("/payInscriptionList/userPayInscriptionList/" + Long.valueOf(0)).locale(
                  Locale.ENGLISH).session(defaultSession)).andExpect(
            view().name("redirect:/payInscriptionList/userPayInscriptionList"));

      mockMvc.perform(
            get("/payInscriptionList/userPayInscriptionList").locale(Locale.ENGLISH).session(
                  defaultSession)).andExpect(view().name("redirect:/payInscriptionList"));
   }
   ...
   public void payUserPayInscriptionTest() throws Exception {
      ...
      mockMvc.perform(
            post("/payInscriptionList/userPayInscriptionList/pay/" + userPayInscription.getId())
                  .locale(Locale.ENGLISH).session(defaultSession)).andExpect(
            view().name("redirect:/payInscriptionList/userPayInscriptionList"));

      // Assert Pay
      assertEquals(
            userPayInscriptionService
                  .findByUserPayInscriptionIds(user.getId(), payInscription.getId()).get(0)
                  .isHasPay(), true);
   }
   ...
   public void blankMessageLoginTest() throws Exception {
      ...
      mockMvc
            .perform(
                  post("/payInscriptionList/userPayInscriptionList").locale(Locale.ENGLISH)
   ...
   public void createPdfALL() throws Exception {
      ...
      PayInscription payInscription2 = new PayInscription("pay of 2015", 2015, Double.valueOf(20),
      ...
      List<UserPayInscription> userPayInscriptions = userPayInscriptionService
            .findByUserPayInscriptionIds(user2.getId(), payInscription2.getId());
      userPayInscriptionService.pay(userPayInscriptions.get(0));

      mockMvc.perform(
            post("/payInscriptionList/userPayInscriptionList/" + payInscription2.getId()).locale(
                  Locale.ENGLISH).session(defaultSession)).andExpect(
            view().name("redirect:/payInscriptionList/userPayInscriptionList"));
